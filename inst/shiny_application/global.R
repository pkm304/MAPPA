...
##generate parameter grids --> now in functions.R
...
##generate parameter combinations (pseudorandom, sobol', latin hypercube) --> now in functions.R
...
##09152017 subranges centered on each selected parameter combination for zoom-in sampling --> now in functions.R
...
#### Modified from a code by  http://arxiv.org/abs/1605.09196 under license GLP-2 --> now in functions.R
...
# rocpr = function(rf.obj,X.test = NULL, ref = NULL, positive){# now in functions.R
#   #library(randomForest)
#   if(!is.null(X.test)){
